...
class Photo {
    ...
    generateThumb(destPath, target, cb) {
        ...
        if (fs.existsSync(target)) {
            ...
            thumb.save(path.join(destPath, this.makeThumb(target)), { quality: 100 })
        }
    }
    file(req, cb) {
    ...
    editAlbum(req, cb) {
        ...
        form.on('field', (name, value) => {
            ...
            fs.generateThumb(destPath, info.thumb)
            ...
            if (info.title !== fields.title) {
                ...
                fs.renameSync(path.join(this.imageDir, fields.year, fields.month, info.title), path.join(this.imageDir, fields.year, fields.month, fields.title))
    ...
    removePhoto(dir, params, cb) {
        if (fs.existsSync(path.join(this.imageDir, dir))) {
            ...
            this.generateThumb(fs.readFileSync(path.join(this.imageDir, params.year, params.month, params.title)), info.thumb)
            info.photos = photos
